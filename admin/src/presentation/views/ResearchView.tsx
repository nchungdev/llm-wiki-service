...
import {
  ...
  PlayCircle, BookOpen, Network, Plus,
  ...
  return (
    <div className="view-panel active no-pad">
      <div className="rv-shell">
        ...
        <main className="rv-main">
          ...
          {isHome && (
            <div className="rv-home">
              ...
              <div className="rv-suggestions-wrap">
                <div className="rv-section-label" style={{ justifyContent: 'center' }}><Sparkles size={12} /> Gợi ý khám phá</div>
          ...
          {!isHome && (
            <div className="rv-workspace">
              ...
              <div className="rv-workspace-bar">
                <button className="rv-icon-btn" onClick={handleNewResearch} title="Quay lại">
                  <ChevronLeft size={16} />
                </button>
                <span className="rv-workspace-query">{messages[0]?.text?.slice(0, 80)}</span>
                <div style={{ display: 'flex', gap: 6, marginLeft: 'auto' }}>
                  {currentSources.length > 0 && (
                    <button
                      className={`rv-icon-btn ${showSources ? 'active' : ''}`}
              ...
              <div className="rv-chat-area">
                ...
                {/* Inline thinking indicator (Quick Search) */}
        ...
        {currentSources.length > 0 && showSources && (
          <aside className="rv-sources-panel">
            <div className="rv-sources-header">
              <span style={{ fontWeight: 700, fontSize: '0.85rem' }}>Nguồn tham khảo</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span className="rv-sources-count">{currentSources.length}</span>
                <button className="rv-icon-btn" onClick={() => setShowSources(false)}><X size={14} /></button>
              </div>
            </div>
